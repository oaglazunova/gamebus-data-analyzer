...
def _load_credentials_from_workbook(
    workbook,
    *,
    source_filename: str,
) -> CredentialStore:
...
def load_participant_credentials(
    ...
    try:
        return _load_credentials_from_workbook(
            workbook,
...
def load_participant_credentials_bytes(
    data: bytes,
    *,
    source_filename: str,
) -> CredentialStore:
    """
    Load an uploaded participant credentials XLSX directly
    from memory.

    The uploaded workbook is not copied into the dataset
    directory.
    """
    if not data:
        raise ValueError(
            "Credentials workbook is empty."
        )
